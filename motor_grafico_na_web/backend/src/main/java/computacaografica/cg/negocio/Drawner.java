package computacaografica.cg.negocio;

import computacaografica.cg.negocio.beans.*;
...
public class Drawner{
	...
	public static ZBuffer[][] renderizar(Objeto objeto, CameraVirtual camera, Iluminacao ilumination, int largura, int altura) throws NegocioException {
    ...
    private static void inicializarDepthBuffer() {
    	...
    	for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				depthBuffer[i][j] = new ZBuffer(new Cor(0,0,0), Double.POSITIVE_INFINITY);
			}
		}
    }
